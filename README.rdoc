= refinerycms-s3assets
 ...
 A common development scenario,  when using a Heroku-hosted RefineryCMS app, is to copy production data to development via the use of heroku db:pull. With this gem one can also copy production assets stored on s3 onto the local filesystem.
